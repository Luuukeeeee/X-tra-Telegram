...
@borg.on(events.NewMessage(pattern=r"\.offline ?(.*)", outgoing=True))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    global USER_AFK  # pylint:disable=E0602
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"Hey Bello sono Occupato!❤ `{total_afk_time}`\Non spammare, eh!🖕🏻😁 " + \
            f"\n\nCercherò di Risponderti al Più presto!🤗🍕\n**REASON**: {reason}" \
            if reason \
            else f"**Belloo!**\nSono Afk!!🎧 For {total_afk_time} I guess.__\n\Ritornerò presto, promesso💞 **( ಠ ʖ̯ ಠ)**  "
